...
<template>
    <div class="pannel group w-full flex flex-col">
        <!-- Pannel content -->
        <div class="pannel-content grow flex transition">
            ...
            <div class="w-full py-1 flex justify-center" v-if="!showPlayer && client.state.player.current">
                <div
        ...
        <div class="basis-12 p-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition duration-200">
            <div class="grid grid-cols-12 gap-2 text-skygray-lighter">
                ...
                <div class="col-span-4 btn-group">
                    ...
                    <button @click="app.shrinkPlayer" title="Shrink player" class="btn text-sm" :class="{ disabled: app.playerMode.size === 'xs' || !showPlayer }">
                        <fa icon="chevron-up" />
                    </button>
                    <button @click="app.expandPlayer" title="Expand player" class="btn text-sm" :class="{ disabled: app.playerMode.size === 'lg' || !showPlayer }">
                        <fa icon="chevron-down" />
                    </button>
                    <button @click="app.setPlayerEnabled(!app.playerMode.enabled)" title="Enable/disable player" class="btn text-sm" :class="{ active: app.playerMode.enabled }">
                ...
                <div class="col-start-9 col-span-4 btn-group">
                    ...
                    <button @click="app.toggleModal('youtubeVideoSearcher')" title="Add a media from Youtube" class="btn text-sm">
                        <fa icon="plus" />
                    </button>
                    <button
