...
import SkyContextMenuItem from '../common/SkyContextMenuItem.vue';
...
<template>
    ...
    <SkyContextMenu v-else>
        ...
        <SkyContextMenuItem v-if="!selected" @select="joinRoom">
            <fa icon="arrow-right-from-bracket" class="w-4 mr-2" />
            Join room
        </SkyContextMenuItem>
        ...
        <SkyContextMenuItem v-if="hasUnread" @select="markAsRead">
            <fa icon="circle-dot" class="w-4 mr-2" />
            Mark as read
        </SkyContextMenuItem>
        <SkyContextMenuItem v-if="!isMuted" @select="client.sendMessage(`/mute ${room.id}`)">
            <fa icon="volume-xmark" class="w-4 mr-2" />
            Mute room
        </SkyContextMenuItem>
        <SkyContextMenuItem v-else @select="client.sendMessage(`/unmute ${room.id}`)">
            <fa icon="bell" class="w-4 mr-2" />
            Unmute room
        </SkyContextMenuItem>
        <SkyContextMenuItem @select="copyRoomId">
            <fa icon="copy" class="w-4 mr-2" />
            Copy room ID
        </SkyContextMenuItem>
    </SkyContextMenu>
